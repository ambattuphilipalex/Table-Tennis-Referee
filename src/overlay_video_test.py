...
VIDEO_PATH = f"/home/shared/Table-Tennis-Referee/data/OpenTT/videos/test/test_{TEST_GAME}.mp4"
OUTPUT_PATH = f"annotated_output_test_{TEST_GAME}.mp4"
MODEL_PATH = "score_predictor_sequential.pth"
CACHE_PATH = f"data/dino_cache/test_{TEST_GAME}/cache.pt"
EVENT_JSON = f"data/OpenTT/annotations/test/test_{TEST_GAME}.json"
...
raw_predictions = []
all_predictions = {}  # frame -> (class, confidence, frames_until)
...
with torch.no_grad():
    for batch in loader:
        ...
        for i, real_frame in enumerate(frame_numbers):
            ...
            all_predictions[real_frame] = (cls, conf, fu)
...
DETECTIONS = [(d["predicted_frame"], "LEFT" if d["event_type"] == 1 else "RIGHT", d["confidence"]) for d in detections]
...
while True:
    ...
    coords = dataset.coords_dict.get(frame_no, None)
    if coords is not None:
        ...
        if fresh > 0.5: 
            ...
            cv2.circle(frame, (px, py), BALL_RADIUS + 2, (0, 0, 0), 2)

    if frame_no in all_predictions:
        cls, conf, fu = all_predictions[frame_no]
        if cls != 0:
            label = "LEFT" if cls == 1 else "RIGHT"
            cv2.putText(frame, f"Pred: {label} in {fu:.0f}f (conf={conf:.2f})", 
                        (30, height - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
